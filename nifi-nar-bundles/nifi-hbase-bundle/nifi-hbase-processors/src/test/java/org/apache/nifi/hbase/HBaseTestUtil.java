...
public class HBaseTestUtil {
    ...
    public static void verifyPut(final byte[] row, final String columnFamily, final Map<String,byte[]> columns, final List<PutFlowFile> puts) {
        boolean foundPut = false;

        for (final PutFlowFile put : puts) {
//            if (!row.equals(put.getRow())) {
            if (!Arrays.equals(row, put.getRow())){	
                continue;
            }
